<div class="row gx-5 mb-3 align-items-center">
    ...
    <div class="col-md-6">
        <div class="row align-items-center">
            <label for="password" class=" col-md-4 form-label">Password<span class="text-danger"><b>*</b></span></label>
            <input class=" col-md-4 form-control"  type="password" id="password" name="password" required value="{{!empty(old('password')) ? old('password') : $user->password ?? ''}}" >
            @error('password')
                <p class="text-danger">{{ $message }}</p>
            @enderror
        </div>
